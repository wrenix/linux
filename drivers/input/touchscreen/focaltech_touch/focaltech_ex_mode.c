...
static int fts_ex_mode_switch(enum _ex_mode mode, u8 value)
{
	...
	case MODE_GLOVE:
		...
		if (ret < 0)
			dev_err(fts_data->dev, "MODE_GLOVE switch to %d fail",
				m_val);
		break;
	...
	case MODE_EDGE:
		...
		if (ret < 0)
			dev_err(fts_data->dev, "MODE_EDGE swith to %d fail",
				m_val);
		break;
	case MODE_COVER:
		ret = fts_write_reg(FTS_REG_COVER_MODE_EN, m_val);
		if (ret < 0)
			dev_err(fts_data->dev, "MODE_COVER switch to %d fail",
				m_val);
		break;
	case MODE_CHARGER:
		ret = fts_write_reg(FTS_REG_CHARGER_MODE_EN, m_val);
		if (ret < 0)
			dev_err(fts_data->dev, "MODE_CHARGER switch to %d fail",
				m_val);
		break;
	default:
