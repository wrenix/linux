...
int fts_read(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
	...
	if (ret < 0) {
		dev_err(fts_data->dev, "data read(addr:%x) %s,status:%x,ret:%d",
			cmd[0],
			    ...
			    u32 readlen)
{
	...
	if (ret < 0) {
		dev_err(fts_data->dev,
			"data read(addr:%x) fail,status:%x,ret:%d", txbuf[0],
			rxbuf[3], ret);
		goto err_spi_dir;
	}
...
int fts_bus_init(struct fts_ts_data *ts_data)
{
	...
	if (NULL == ts_data->bus_tx_buf) {
		dev_err(fts_data->dev,
			"failed to allocate memory for bus_tx_buf");
		return -ENOMEM;
	}
	...
	if (NULL == ts_data->bus_rx_buf) {
		dev_err(fts_data->dev,
			"failed to allocate memory for bus_rx_buf");
		return -ENOMEM;
	}
